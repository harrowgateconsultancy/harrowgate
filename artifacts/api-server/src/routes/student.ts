...
import { uploadToWebDav } from "../lib/webdavService";
...
router.post("/student/submissions/:id/receipt", requireStudentAuth, async (req: any, res) => {
  try {
    ...
    uploadToWebDav({ submissionId, studentName: submission.name, fileName, fileUrl, documentType: "payment_receipt" }).catch(() => {});
    res.json(doc);
...
router.post("/student/submissions/:id/additional-docs", requireStudentAuth, async (req: any, res) => {
  try {
    ...
    uploadToWebDav({ submissionId, studentName: submission.name, fileName, fileUrl, documentType: "additional_doc" }).catch(() => {});
    res.json(doc);
...
router.post("/student/submissions/:id/receipt2", requireStudentAuth, async (req: any, res) => {
  try {
    ...
    uploadToWebDav({ submissionId, studentName: submission.name, fileName, fileUrl, documentType: "second_payment_receipt" }).catch(() => {});
    res.json(doc);
...
router.post("/student/submissions/:id/receipt3", requireStudentAuth, async (req: any, res) => {
  try {
    ...
    uploadToWebDav({ submissionId, studentName: submission.name, fileName, fileUrl, documentType: "final_payment_receipt" }).catch(() => {});
    res.json(doc);
