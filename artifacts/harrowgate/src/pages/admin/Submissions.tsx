...
const docTypeLabels: Record<string, string> = {
  edu_1: "Doc 1 — Degree / Diploma",
  edu_2: "Doc 2 — Transcript",
  edu_3: "Doc 3 — Previous Qualification",
  edu_4: "Doc 4 — Language Certificate",
  edu_5: "Doc 5 — Supporting Document",
...
function fileIcon(fileName: string, mimeType?: string | null) {
  if (isPdf(fileName, mimeType)) return "📄";
  if (isImage(fileName, mimeType)) return "🖼️";
  return "📎";
}
...
export default function Submissions() {
  ...
  const deleteDoc = useMutation({
    mutationFn: async ({ submissionId, docId }: { submissionId: number; docId: number }) => {
      const res = await fetch(`${getApiBase()}/api/admin/student-submissions/${submissionId}/documents/${docId}`, {
        method: "DELETE",
      });
      if (!res.ok) throw new Error("Failed");
    },
    onSuccess: () => {
      ...
      qc.fetchQuery({
        ...
        if (selected) {
          const fresh = all.find(s => s.id === selected.id);
  ...
  const viewUrl = (doc: Document) =>
    `${getApiBase()}/api/admin/student-submissions/${selected?.id}/documents/${doc.id}/view`;
  const downloadUrl = (doc: Document) =>
    `${getApiBase()}/api/admin/student-submissions/${selected?.id}/documents/${doc.id}/download`;
    ...
    <div className="min-h-screen" style={{ background: BG }}>
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        ...
        <div className="space-y-3">
          {filtered.map(s => {
            ...
            return (
              <div key={s.id}
                className="rounded-2xl border transition-all cursor-pointer hover:border-opacity-60"
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="w-full max-w-3xl rounded-3xl border flex flex-col"
            style={{ background: "#0a1f0e", borderColor: "rgba(162,137,89,0.18)", maxHeight: "92vh" }}>
                ...
                <div className="mx-6 mt-5 rounded-2xl border overflow-hidden" style={{ borderColor: "rgba(162,137,89,0.15)", background: "rgba(0,0,0,0.3)" }}>
                  ...
                  <div className="p-2" style={{ minHeight: 260 }}>
                    {isImage(previewDoc.fileName, previewDoc.mimeType) ? (
                      <img src={viewUrl(previewDoc)} alt={previewDoc.fileName}
                        className="max-w-full mx-auto rounded-xl" style={{ maxHeight: 420, objectFit: "contain" }} />
                      ...
                      <div className="flex flex-col items-center justify-center py-12 gap-4">
                        ...
                        <p className="text-sm" style={{ color: "rgba(162,137,89,0.55)" }}>Preview not available for this file type.</p>
                        <a href={downloadUrl(previewDoc)} download={previewDoc.fileName}
              ...
              <div className="px-6 py-5 space-y-6">
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-sm font-semibold" style={{ color: GOLD }}>Documents ({selected.documents.length})</p>
                  </div>

                  {selected.documents.length === 0 && (
                    <div className="rounded-xl py-6 text-center border" style={{ borderColor: "rgba(162,137,89,0.08)", background: "rgba(162,137,89,0.03)" }}>
                  ...
                  <div className="space-y-2">
                    {/* Student documents */}
                    {selected.documents.filter(d => d.documentType !== "payment_receipt" && !d.documentType.startsWith("admin_")).map(doc => (
                      <DocRow key={doc.id} doc={doc} label={docTypeLabels[doc.documentType] || doc.documentType}
                    ...
                    {selected.documents.filter(d => d.documentType.startsWith("admin_")).map(doc => (
                      <DocRow key={doc.id} doc={doc} label={doc.documentType === "admin_doc" ? "Admin Document" : doc.documentType}
                        onPreview={() => setPreviewDoc(previewDoc?.id === doc.id ? null : doc)}
                        onDownload={() => window.open(downloadUrl(doc), "_blank")}
                        onDelete={() => { if (confirm(`Delete "${doc.fileName}"?`)) deleteDoc.mutate({ submissionId: selected.id, docId: doc.id }); }}
                        isActive={previewDoc?.id === doc.id}
                ...
                <div className="rounded-2xl border overflow-hidden" style={{ background: "rgba(0,0,0,0.2)", borderColor: "rgba(162,137,89,0.12)" }}>
                  <div className="px-4 py-3 border-b" style={{ borderColor: "rgba(162,137,89,0.08)" }}>
                    ...
                    <p className="text-xs mt-0.5" style={{ color: "rgba(162,137,89,0.4)" }}>Add a document to this student's file (e.g. application form, approval letter)</p>
                  </div>
                ...
                <button
                  onClick={() => updateStatus.mutate({ id: selected.id, status: selected.status, adminNotes: notes })}
                  disabled={updateStatus.isPending}
...
function DocRow({ doc, label, onPreview, onDownload, onDelete, isActive, tagColor, tagTextColor, tag }: {
  ...
  return (
    <div className="rounded-xl border transition-all"
      style={{ background: isActive ? "rgba(162,137,89,0.07)" : "rgba(162,137,89,0.03)", borderColor: isActive ? "rgba(162,137,89,0.28)" : "rgba(162,137,89,0.1)" }}>
      <div className="px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button onClick={onDelete}
            className="px-2.5 py-1.5 rounded-lg text-xs font-medium border transition-all hover:opacity-80"
